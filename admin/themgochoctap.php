...
if($id && $act=='del'){
    $gochoctap->id = $id; $t = $gochoctap->get_one();
    if($t['hinhanh']){
    ...
    if($gochoctap->delete()) transfers_to('gochoctap.html?msg=Xóa thành công!');
}
$list = $gochoctap->get_all_list();

if($id && $act == 'edit'){
    $gochoctap->id = $id; $t = $gochoctap->get_one();
    $id_danhmucgochoctap = $t['id_danhmucgochoctap'];
    $tieude = $t['tieude'];
    $mota = $t['mota'];
    $noidung = $t['noidung'];
    $hinhanh = $t['hinhanh'];
...
if(isset($_POST['submit'])){
    ...
    $id_danhmucgochoctap = isset($_POST['id_danhmucgochoctap']) ? $_POST['id_danhmucgochoctap'] : '';
    ...
    $gochoctap->id_danhmucgochoctap = $id_danhmucgochoctap;
    $gochoctap->tieude = $tieude;
    $gochoctap->mota = $mota;
    $gochoctap->noidung = $noidung;
    $gochoctap->hinhanh = $arr_hinhanh;
    $gochoctap->hienthi = $hienthi;
    $gochoctap->video = $video;
    $gochoctap->orders = $orders;

    if($act == 'edit'){
        $gochoctap->id = $id;
        if($gochoctap->edit()) transfers_to('gochoctap.html?msg=Chỉnh sửa thành công');
    } else {
        if($gochoctap->insert()) transfers_to('gochoctap.html?msg=Thêm thành công');
    }
}
?>
	...
	<div class="col-md-12">
		<div class="panel panel-primary">
            <div class="panel-heading">
                ...
                <h4 class="panel-title"><i class="fa fa-gears"></i> Nhập thông tin Góc học tập</h4>
            </div>
            ...
            <div class="panel-footer">
                <a href="gochoctap.html" class="btn btn-white"><i class="fa fa-reply-all"></i> Trở về</a>
